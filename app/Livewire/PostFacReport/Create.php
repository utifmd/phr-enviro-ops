...
class Create extends BaseComponent
{
    ...
    private function assignLog(string $postId): void
    {
        ...
        $this->logRepos->addLogs(
            '/post-fac-report/requests/'.$postId, $highlight
        );
    }
    ...
    private function pairInfoDetailToPlanTripState(array $infos, Collection $details): array
    {
        ...
        foreach ($infos as $trip) {
            $trip['type'] = PostFacReportTypeEnum::PLAN->value;
            $trip['status'] = PostFacReportStatusEnum::APPROVED->value;
            ...
            foreach ($details as $detail) {
                ...
                $trip['details'][] = $payload; // $payload['url'] = route('post-fac.show', $detail->id);
            }
            if (!isset($trip['act_name'])) {
                Log::error('act_name is not set in trip', $trip);
                continue;
            }
            $trip['url'] = $trip['act_name'] == ActNameEnum::Outgoing->value ? route('post-fac-out.create') : route('post-fac-in.create');
    ...
    private function mapTripState(array $tripState): array
    {
        ...
        foreach ($tripState as $trip) {
            ...
            $trip['type'] = PostFacReportTypeEnum::ACTUAL->value;
    ...
    private function approveVTSubmission(): void
    {
        ...
        $builder->update(['status' => PostFacReportStatusEnum::APPROVED->value]);
    }
    ...
    {
        return view('livewire.post-fac-report.create');
    }
}
