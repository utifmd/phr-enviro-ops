<div class="w-3/12 p-4 bg-white bg-opacity-45 rounded-lg border">
    <div class="flex flex-col items-start space-y-2">
        ...
        <p class="text-sm font-medium text-gray-500">{{ $data['scope_name'] }}</p>
        <p class="text-2xl font-semibold text-gray-800">&#8627; {{ $data['count'] }} load{{ $data['count'] > 1 ? 's' : ''}}</p>
        <hr class="w-full border-t border-gray-200">
        {{--<span class="text-sm text-gray-500">latest update <small>0 minutes ago</small></span>--}}
        <a href="#" class="text-sm font-medium text-indigo-600 hover:underline flex items-center">
            Detail
