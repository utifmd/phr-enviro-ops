            ...
            <div class="w-full">
                <div class="sm:flex space-y-2 md:space-y-0">
                    <div class="sm:flex-auto">
                        ...
                        @endif
                        <p class="mt-2 text-xs text-gray-800">Facility
                            Representative: {{ $form->postModel->user->email ?? 'deleted account' }}</p>
                    </div>
                                        ...
                                        <tr class="even:bg-gray-50" wire:key="{{ $trip['id'] }}">
                                            ...
                                            @can(\App\Policies\UserPolicy::IS_USER_IS_FAC_REP)
                                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 space-x-1">
                                                    <button wire:loading.attr="disabled"
                                                            wire:click.prevent="onChangeStatus('{{$trip['id']}}', '{{\App\Utils\PostFacReportStatusEnum::REJECTED->value}}')"
                                                            class="px-3 border border-red-600 rounded text-red-600 hover:opacity-50">{{ __('Deny') }}</button>
                                                    <button wire:loading.attr="disabled"
                                                            wire:click.prevent="onChangeStatus('{{$trip['id']}}', '{{\App\Utils\PostFacReportStatusEnum::APPROVED->value}}')"
