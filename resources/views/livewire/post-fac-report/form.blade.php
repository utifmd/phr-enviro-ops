<div x-data="{ isAnotherDateOpen: false }" class="space-y-6">
    <div class="block md:grid md:grid-cols-4 space-y-4 md:space-y-0 gap-8">

        <div x-show="isAnotherDateOpen">
            <x-input-label @click="isAnotherDateOpen = !isAnotherDateOpen"
                           class="cursor-pointer font-bold text-green-800" for="date" value="Another Date"/>
        ...
        <div x-show="!isAnotherDateOpen">
            <x-input-label @click="isAnotherDateOpen = !isAnotherDateOpen"
                           class="cursor-pointer font-bold text-green-800" for="date"
                           value="{{ 'Date'.($currentDate == date('Y-m-d') ? ' (Today)' : '') }}"/>
            <x-text-input
        ...
        <div>
            ...
            <x-select-option class="mt-1 block w-full" wire:model="form.time" wire:change.prevent="onTimeOptionChange"
    ...
    @include('livewire.post-fac-report.tabled')
    {{--<div>
