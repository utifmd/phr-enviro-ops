@props([
    'drilling' => \App\Utils\PostFacTypeEnum::DRILLING->value,
    'mudPit' => \App\Utils\PostFacTypeEnum::MUD_PIT->value,
    'concentratedWater' => \App\Utils\PostFacTypeEnum::CONCENTRATED_WATER->value,
    'treatedWater' => \App\Utils\PostFacTypeEnum::TREATED_WATER->value,
])
<div x-data="{
 formState:{
    isDrilling: true,
    ...
    <x-loading-field label="Transporter" for="transporter" required="true">
        <x-select-option
                id="transporter" name="transporter" type="text" class="mt-1 block w-full" autocomplete="transporter"
    ...
    <x-loading-field label="Driver Name" for="driver" required="true">
        <x-select-option
                id="driver" name="driver" type="text" class="mt-1 block w-full" autocomplete="driver"
                placeholder="Driver"
    ...
    <x-loading-field label="Police Number" for="police_number" required="true">
        <x-text-search-option
                id="police_number" name="police_number" type="text" class="mt-1 block w-full"
    ...
    <div>
        ...
        <x-select-option wire:model="form.facility" :cases="$facilities" id="facility" name="facility" type="text"
    ...
    <div>
        ...
        <div id="type" class="flex flex-wrap ms-2 space-x-6">
            <div class="flex items-center">
                ...
                <label for="red-radio"
                       class="ms-2 text-sm font-medium text-gray-900 darktext-gray-300">{{$drilling}}</label>
            </div>
            <div class="flex items-center">
                ...
                <label for="green-radio"
                       class="ms-2 text-sm font-medium text-gray-900 darktext-gray-300">{{$mudPit}}</label>
            </div>
        </div>
        @error('form.type')
        <x-input-error class="mt-2" :messages="$message"/>
    ...
    <x-loading-field for="well_name" label="Well Name" required="true">
        <x-text-search-option
            ...
            <template x-for="(well, i) in formState.wells"
                      :key="i"> {{--<li x-on:click="open = false; formState.wellsCollected.push(well); $wire.onWellSelected(well)"--}}
                <li x-on:click="
                    open = false;
                    formState.wellsCollected[0] = well;
                    $wire.onWellSelected(well)"
                    x-text="well.ids_wellname"
                    class="px-4 py-2 hover:bg-indigo-100 cursor-pointer"></li> {{--formState.wellsCollected.map(({rig_no}) => rig_no).join(', ')--}}
            </template>
        </x-text-search-option>
        <div>
            <template id="wells" name="wells" x-for="(well, j) in formState.wellsCollected" :key="j">
                <x-text-badge-with-close-button
                        x-text="well.ids_wellname+', '+well.legal_well+', '+well.well_number+', '+well.field_name"
                        class="my-1 mx-1"/>
            </template>
        </div>
        @error('well_name')
    ...
    <x-loading-field for="rig_name" label="Rig Name" required="true">
        <x-text-input
                x-model="formState.rigName" disabled="true" id="rig_name" name="rig_name" type="text"
                class="mt-1 block w-full"
    ...
    <div>
        ...
        <x-text-input wire:model="form.tds" id="tds" name="tds" type="number" pattern="[0-9]*" inputmode="numeric"
                      min="0" class="mt-1 block w-full" autocomplete="tds"
                      placeholder="Tds"/>
        @error('form.tds')
        <x-input-error class="mt-2" :messages="$message"/>
        @enderror
    ...
    <div>
        ...
        <x-text-input wire:model="form.volume" id="volume" name="volume" type="number" pattern="[0-9]*"
                      inputmode="numeric" min="0" class="mt-1 block w-full"
    ...
    <div>
        ...
        <x-text-input wire:model="form.load" id="load" name="load" type="number" pattern="[0-9]*" inputmode="numeric"
                      min="1" max="999" class="mt-1 block w-full"
