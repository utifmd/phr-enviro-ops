...
<div>
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <x-select-option :cases="\App\Utils\ActNameEnum::cases()" wire:model="workTripForm.act_name" wire:change="onActivityChange" id="act_name" name="act_name" />
        </div>
        @if(!empty($workTripForm->act_name))
            <div>
                <x-input-label for="act_process" :value="__('Activity')"/>
                <x-select-option :cases="$processes" wire:model="workTripForm.act_process" id="act_process" name="act_process" />
            </div>
            <div class="flex space-x-2">
                <div class="w-6/12">
                    <x-input-label for="act_value" :value="__('Quantity')"/>
                    <x-text-input type="number" wire:model="workTripForm.act_value" wire:change="onActValueChange" id="act_value" name="act_value" placeholder="Qty"/>
                ...
                <div class="w-6/12">
                    ...
                    <x-text-input type="text" :disabled="true" wire:model="workTripForm.act_unit" id="act_unit" name="act_unit" required/>
                </div>
            </div>
            <div class="m-auto">
                <button type="button"  wire:click="onAddActivityPressed" class="whitespace-nowrap px-3 py-2 text-xs font-medium text-center inline-flex items-center rounded-full text-white bg-green-700 hoverbg-green-800 focusring-4 focusoutline-none focusring-blue-300 darkbg-blue-600 darkhoverbg-blue-700 darkfocusring-blue-800">
        ...
        <table class="w-full divide-y divide-gray-300 mt-6">
            ...
            <tr>
                ...
                @foreach ($workTripsState as $i => $workTrip)
                    <th wire:click="onTableHeaderPressed({{$i}})" scope="col" class="py-3 pl-4 pr-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 hover:cursor-pointer hover:line-through hover:text-red-600 hover:font-semibold">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <x-select-option wire:model="form.destination" wire:change="onDestChange" :cases="$locations" id="destination" name="destination"/>
        ...
        <div>
            ...
            <x-text-input wire:model="form.description" id="description" name="description" type="text" class="mt-1 block w-full"  placeholder="Description" /> {{--rows="3"--}}
            <div class="grid grid-cols-2 gap-1 my-1">
