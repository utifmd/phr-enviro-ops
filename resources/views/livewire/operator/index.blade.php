            ...
            <div class="w-full">
                ...
                <div class="sm:flex space-y-2 md:space-y-0">
                    <div class="sm:flex-auto">
                        <h1 class="text-base font-semibold leading-6 text-gray-900">{{ __('Operators') }}</h1>
                        <p class="mt-2 text-sm text-gray-700">A list of all the {{ __('Operators') }}.</p>
                    </div>
                    <div class="min-w-md">
                    ...
                    <div class="min-w-md md:mx-2">
                        ...
                        <x-dropdown id="options" align="right" width="48">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('operators.create', ['departments' => $departments])" class="text-green-600 cursor-pointer">
                                    {{ __('Add New Operator') }}
                                </x-dropdown-link>
                            </x-slot>
                        </x-dropdown>
                                ...
                                @foreach ($operators as $operator)
                                    <tr class="even:bg-gray-50" wire:key="{{ $operator->id }}">
                                        ...
                                        <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900">
                                            <a wire:navigate href="{{ route('operators.show', $operator->id) }}" class="text-gray-600 font-bold hover:text-gray-900 mr-2">{{ __('Show') }}</a>
                                            <a wire:navigate href="{{ route('operators.edit', $operator->id) }}" class="text-indigo-600 font-bold hover:text-indigo-900  mr-2">{{ __('Edit') }}</a>
                                            <button
