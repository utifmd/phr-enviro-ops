            ...
            <div class="w-full divide-y divide-gray-300">
                <table wire:loading.class="opacity-50" class="w-full divide-y divide-gray-300">
                    ...
                    <tr>
                        ...
                        <th scope="col"
                            class="py-3 pl-4 pr-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Unit
                        </th>
                    </tr>
                    </thead>
                    <tbody wire:loading.attr="opacity-50" x-data="{ infoState: @entangle('infoState'), delInfoQueue: @entangle('delInfoQueue'), formActValue: @entangle('form.act_value') }" class="divide-y divide-gray-200 bg-white">
                        <template x-for="(info, i) in infoState" :key="i">
                            ...
                            <tr wire:key="i" class="even:bg-gray-50 hover:bg-amber-50">
                                ...
                                <td x-on:click="$wire.onRemoveInfoState(info); infoState.splice(i, 1);" x-text="info.act_name+' '+info.act_process" class="cursor-pointer whitespace-nowrap py-4 pl-4 pr-3 text-sm font-semibold text-gray-900 hover:line-through hover:font-semibold hover:text-red-600"></td>
                                ...
                                <td>
                                    <x-text-input
                                        x-model="info.act_value" id="act_value" name="act_value" type="number" pattern="[0-9]*"
                                        inputmode="numeric" min="0" max="999" autocomplete="act_value"
                                        placeholder="Quota"/>
                                </td>
                                <td x-text="info.act_unit" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-semibold text-gray-900"></td>
                            </tr>
                        </template>
