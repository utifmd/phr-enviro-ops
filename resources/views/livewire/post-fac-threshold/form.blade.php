<div class="space-y-6">
    <div x-data="{ open: false }" class="block md:grid md:grid-cols-4 space-y-4 md:space-y-0 gap-8">
        @if(is_null($dateParam))
            <div x-show="open">
                <x-input-label @click="open = !open" class="cursor-pointer text-green-800" for="date"
                               value="Another Date"/>
                <x-text-input
        ...
        <div>
            ...
            <x-select-option wire:model="form.time" class="mt-1 block w-full" wire:change.prevent="onTimeOptionChange"
        ...
        {{--<div>
            ...
            <x-text-input wire:model="form.act_value" id="act_value" name="act_value" type="number" pattern="[0-9]*"
                          inputmode="numeric" min="0" max="999" class="mt-1 block w-full" autocomplete="act_value"
                          placeholder="Quota"/>
        ...
        {{--<div>
            ...
            <button id="btn_apply" name="btn_apply" type="button" wire:click="onStateInfoPressed"
                    class="mt-1 block p-3 text-xs font-medium text-center items-center rounded-full text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
    ...
    @include('livewire.post-fac-threshold.tabled')
    {{--@foreach($timeOptions as $timeOpt)
        @include('livewire.post-fac-threshold.tabled', ['timeOpt' => $timeOpt['value']])
    @endforeach--}}
