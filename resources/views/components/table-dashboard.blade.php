    ...
    <tr>
        @if(isset($recapPerMonth['days']))
            @foreach($recapPerMonth['days'] as $day)
                <th scope="col" class="py-4 text-xs text-center font-semibold uppercase tracking-wide text-gray-500">{{ $day }}</th>
            @endforeach
        @endif
    </tr>
    </thead>
    ...
    @if(isset($recapPerMonth['data']))
        @foreach($recapPerMonth['data'] as $load)
            ...
            <tr>
                ...
                <td></td>
                @foreach($load['days'] as $v)
                    <td></td>
                @endforeach
                <td></td>
                <td class="whitespace-nowrap px-3 py-4 text-center text-sm font-semibold text-gray-900">{{ $recapPerMonth['grand_total'] }}</td>
            </tr>
        @endforeach
    @endif
    </tbody>
